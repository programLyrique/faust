...
static Tree addRateEnv(Tree s1, int r1, Tree E)
{
    if (isList(E)) {
        ...
        return cons(cons(s1, tree(r1)), gGlobal->nil);
    }
}
...
static Tree flatRateEnvironmentList(Tree lre)
{
    ...
    while (isList(lre)) {
        ...
        faustassert(success);  // can't failed if environments are independant
        lre = tl(lre);
    }
    return e;
}
...
//Needed because otherwise problems with the order of initialization of global variables
static property<Tree>& initInferreRateProperty()
{
    static property<Tree> gInferreRateProperty;
    return gInferreRateProperty;
}


static void setInferreRateProperty(property<Tree>& gInferreRateProperty, Tree sig, int rate, Tree renv)
{
    gInferreRateProperty.set(sig, cons(tree(rate), renv));
}

static bool getInferreRateProperty(property<Tree>& gInferreRateProperty, Tree sig, int* rate, Tree& renv)
{
    Tree p;
...
static void inferreRate(Tree sig, int* rate, Tree& E)
{
    TRACE(cerr << ++TABBER << "inferreRate(" << ppsig(sig) << ")" << endl);
     property<Tree>& gInferreRateProperty = initInferreRateProperty();
    if (!getInferreRateProperty(gInferreRateProperty, sig, rate, E)) {
        doInferreRate(sig, rate, E);
        setInferreRateProperty(gInferreRateProperty, sig, *rate, E);
    }
...
static void doInferreRate(Tree sig, int* rate, Tree& E)
{
    ...
    if (isSigVectorize(sig, x, w)) {
        ...
        *rate = 1;
        E     = addRateEnv(sig, 1, gGlobal->nil);
